...
var ConfigurationStore = (_dec = (0, _boot.store)('ConfigurationStore'), _dec(_class = (_class2 = function () {
  function ConfigurationStore() {
    ...
    this.loadService = function () {
      return _boot.axios.get('manager/v1/services');
    };

    this.deleteConfig = function (configId) {
      return _boot.axios['delete']('manager/v1/configs/' + configId);
    };

    this.setDefaultConfig = function (configId) {
      return _boot.axios.put('manager/v1/configs/' + configId + '/default');
    };

    this.createConfig = function (data) {
      return _boot.axios.post('manager/v1/configs', JSON.stringify(data));
    };

    this.getEditConfigData = function (id) {
      return _boot.axios.get('manager/v1/configs/' + id);
    };

    this.versionCheck = function (data) {
      return _boot.axios.post('manager/v1/configs/check', JSON.stringify(data));
    };
  } // 当前配置id
  // 联动service
  ...
  (0, _createClass3['default'])(ConfigurationStore, [{
    ...
    value: function loadCurrentServiceConfig(serviceId) {
      ...
      _boot.axios.get('/manager/v1/configs?' + _queryString2['default'].stringify(queryObj)).then(function (data) {
        return _this.setCurrentServiceConfig((data.list || []).slice());
      });
    }
  }, {
    key: 'modifyConfig',
    value: function modifyConfig(configId, type, data) {
      return _boot.axios.put('manager/v1/configs/' + configId + '?type=' + type, data);
    }
  }, {
    key: 'getStatus',
    get: function get() {
