...
var APIOverviewStore = (_dec = (0, _boot.store)('APIOverviewStore'), _dec(_class = (_class2 = function () {
  function APIOverviewStore() {
    ...
    this.loadFirstChart = function () {
      return _boot.axios.get('/manager/v1/swaggers/api/count').then(function (data) {
        if (data.failed) {
    ...
    this.loadSecChart = function (beginDate, endDate) {
      return _boot.axios.get('/manager/v1/swaggers/service_invoke/count?begin_date=' + beginDate + '&end_date=' + endDate).then(function (res) {
        if (res.failed) {
    ...
    this.loadServices = function () {
      return _boot.axios.get('/manager/v1/swaggers/resources');
    };
    ...
    this.loadThirdChart = function (beginDate, endDate, service) {
      ...
      return _boot.axios.get('/manager/v1/swaggers/api_invoke/count?' + _queryString2['default'].stringify(queryObj)).then(function (data) {
        if (data.failed) {
          Choerodon.prompt(data.message);
        } else {
          if (data.api.length) {
